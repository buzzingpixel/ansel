...
function container-ansel-php74() {
    ...
    if [[ -z "${allArgsExceptFirst}" ]]; then
        ...
        docker exec ${interactiveArgs} -e XDEBUG_MODE=off -w /var/www ansel-php74 bash;
    else
        docker exec ${interactiveArgs} -w /var/www ansel-php74 bash -c "XDEBUG_MODE=off ${allArgsExceptFirst}";
    fi
